...
- 以唯一键字段作为条件，把源表中的记录插入目的表
- 以唯一键字段作为条件，删除源表中的记录

如果表中有blob字段，那么每批迁移的记录数不宜过大。

在迁移数据的过程中，如果出现唯一键冲突，一条记录的失败会导致整批迁移失败

### 建表

```sql
drop index IDX_ZHOBTMIND_HIS_3 on T_ZHOBTMIND_HIS;

drop index IDX_ZHOBTMIND_HIS_2 on T_ZHOBTMIND_HIS;

drop index IDX_ZHOBTMIND_HIS_1 on T_ZHOBTMIND_HIS;

drop table if exists T_ZHOBTMIND_HIS;
...
(
   ...
   unique key ZHOBTMIND_HIS_KEYID (keyid)
);

alter table T_ZHOBTMIND_HIS comment '本表用于migtable程序的测试，表结构与T_ZHOBTMIND相同，但删除了外键约束。';
