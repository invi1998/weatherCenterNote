...
/project/tools/bin/procctl   30 /project/tools/bin/dminingmysql /log/idc/dminingmysql_ZHOBTMIND_vip2.log "<connstr>127.0.0.1,root,psaaword,mysql,3306</connstr><charset>utf8</charset><selectsql>select obtid,date_format(ddatetime,'%%Y-%%m-%%d %%H:%%i:%%s'),t,p,u,wd,wf,r,vis,keyid from T_ZHOBTMIND where keyid>:1 and ddatetime>timestampadd(minute,-65,now())</selectsql><fieldstr>obtid,ddatetime,t,p,u,wd,wf,r,vis,keyid</fieldstr><fieldlen>10,19,8,8,8,8,8,8,8,15</fieldlen><bfilename>ZHOBTMIND</bfilename><efilename>HYCZ</efilename><outpath>/idcdata/xmltodb/vip2</outpath><incfield>keyid</incfield><timeout>30</timeout><pname>dminingmysql_ZHOBTMIND_vip2</pname><maxcount>1000</maxcount><connstr1>127.0.0.1,root,psaaword,mysql,3306</connstr1>"
...
void _help(char *argv[])
{
    ...
    printf("Sample:/project/tools/bin/procctl 10 /project/tools/bin/xmltodb_oracle /log/idc/xmltodb_oracle_vip2.log \"<connstr>invi/psaaword@snorcl11g_130</connstr><charset>Simplified Chinese_China.AL32UTF8</charset><inifilename>/project/idc/ini/xmltodb.xml</inifilename><xmlpath>/idcdata/xmltodb/vip2</xmlpath><xmlpathbak>/idcdata/xmltodb/vip2bak</xmlpathbak><xmlpatherr>/idcdata/xmltodb/vip2err</xmlpatherr><timetvl>5</timetvl><timeout>50</timeout><pname>xmltodb_oracle_vip2</pname>\"\n\n");
...
- keyid字段的处理
- 时间函数和时间格式
- Oracle不需要为查询语句创建专用的数据库链接（这是因为只有mysql的查询语句会有如果next结果集没有读取完，就不能执行任何sql语句的特性，所以mysql需要额外创建一个链接，一个用于sql查询，一个用于sql删除）
- Oracle不需要MAXPARAM这个宏
