...
每次只同步数据表中新增的记录。增量同步的效率很高，但是对同步的数据表有两个要求
...
![](./img/QQ截图20220420231332.png)

数据同步程序同时连接本地和远程数据库，先从远程表中查询待同步的数据，放在内存变量中，然后再插入本地表中
...
测试不在同一个数据库和在同一个数据库下，采用federated表同步和不采用federated同步的性能比对

![](./img/QQ截图20220420235107.png)

如图，红色两个分别是在不同的数据库（一个远程，一个本地），是否采用federated表同步没有区别。蓝色是在同一个数据库进行数据同步，可以看到，不采用federated表同步的方式还要快一些

不采用federated引擎可以实现在不同的数据库产品和版本之间进行同步（MySQL->Oracle）

**同步模块的不足**

数据略有延迟，但是在业务可接受的范围内

对远程数据库会造成压力（解决思路就是采用MySQL的binlog来进行数据同步）

![](./img/QQ截图20220421001341.png)
