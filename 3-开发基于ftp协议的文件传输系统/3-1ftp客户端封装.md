...
在上面详细探讨了文件下载的流程之后，反过来看文件上传其实也就变得简单很多，他们的步骤其实也都一样。

![](./img/QQ截图20220404222345.png)

先把上次已经成功上传的文件加载到容器1中，然后使用Dir获取本地待上传的文件放入到容器2中，再把容器2和容器1对比，不需要上传的文件存放在容器3中，需要上传的文件存放在容器4中。最后再把容器4中的文件一一上传即可。

然后下面是两个代码测试

![](./img/QQ截图20220404235213.png)

![](./img/QQ截图20220405000041.png)
