...
- 在主线程中预先创建n个工作线程
- 主线程监听客户端的连接，当有新的客户端连上来的时候，把它放入待处理队列
- 工作线程从队列中取出客户端连接，处理客户端的业务请求
- 在队列中，存放的是客户端的socket
- 采用条件变量+互斥锁（生产消费者模型）实现队列功能

一个核一个时间只能运行一个线程，线程少了不能有效利用多核的优势，线程多了没有CPU给你运行。还有，如果线程太多，在线程之间切换也会浪费时间，所以线程不是越多越好

线程并不是一直在跑，可能会阻塞等待，等待的时候，CPU是空闲的，如果线程和CPU核数相同，就会有资源浪费的情况，如果线程数略多，可以把其他的线程浪费的CPU利用起来
